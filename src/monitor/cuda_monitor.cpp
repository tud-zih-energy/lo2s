...
: PollMonitor(trace, "CUDAMonitor", config().ringbuf_read_interval),
  ...
  local_cctx_tree_(trace.create_local_cctx_tree(trace.cuda_writer(process_)))
{
}
...
{
    local_cctx_tree_.cctx_leave(last_tp_, CallingContext::process(process_));
...
void CUDAMonitor::monitor(int fd [[maybe_unused]])
{
    while (!ringbuf_reader_.empty())
    {
        ...
        if (event_type == (uint64_t)cuda::EventType::KERNEL)
        {
            ...
            local_cctx_tree_.cctx_leave(end_tp, CallingContext::cuda(kernel->kernel_id));
        }
