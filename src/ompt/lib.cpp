...
{
    ompt_rb_writer->ompt_enter(ompt_rb_writer->timestamp(), lo2s::OMPTType::PARALLEL, reinterpret_cast<uint64_t>(codeptr_ra));
}

static void on_ompt_callback_parallel_end(ompt_data_t* parallel_data, ompt_data_t* task_data,
                                          int flag, const void* codeptr_ra)
{
    ompt_rb_writer->ompt_leave(ompt_rb_writer->timestamp());
}
...
int ompt_initialize(ompt_function_lookup_t lookup, int initial_device_num, ompt_data_t* tool_data)
{
    pid_t pid = getpid();
    ompt_rb_writer =
        std::make_unique<lo2s::RingbufWriter>(lo2s::Process(pid), lo2s::RingbufMeasurementType::OPENMP);
