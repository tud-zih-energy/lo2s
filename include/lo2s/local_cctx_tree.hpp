...
public:
    ...
    void cctx_enter(const otf2::chrono::time_point& tp, const CallingContext& ctx,
                    const Cctxs&... ctxs)
    {
        cctx_enter(tp, cur_.size(), 0, ctx, ctxs...);
    }

    template <typename... Cctxs>
    void cctx_enter(const otf2::chrono::time_point& tp, const CallingContext& ctx)
    {
        cctx_enter(tp, cur_.size(), 0, ctx);
    }
    ...
    {
        cctx_leave(tp, cur_.size() - 1, cctx);
    }
    ...
    void cctx_leave(otf2::chrono::time_point tp, uint64_t level, const CallingContext& cctx)
    {
        if (cur_.size() <= level)
        {
            ...
            else
            {
                Log::debug() << "Trying to leave" << cctx.name() << " on level " << level
                             << " of the callstack, but there are only " << cur_.size()
                             << " elements on it!";

                if (cur_.back()->first != CallingContext::root())
                {
        ...
        {
            Log::debug() << "Trying to exit " << cctx.name() << " but " << cur_[level]->first.name()
                         << " is on the callstack at level " << level << "!";
            writer_.write_calling_context_leave(tp, cur_.back()->second.ref);
            cur_.erase(++cur_.begin(), cur_.end());
        }
        else
        {
            writer_.write_calling_context_leave(tp, cur_.back()->second.ref);
            cur_.erase(cur_.begin() + level, cur_.end());
        }
    }
...
private:
                                ...
                                uint64_t& unwind_distance, const CallingContext& cctx)
    {
        ...
        if (unwind_distance != 0 || cur_.size() == level)
        {
        ...
        {
            if (cur_.size() > level && cur_[level]->first != cctx)
            {
