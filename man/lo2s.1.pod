...
Wake up interval based monitors (e.g. x86_adapt, x86_energy) every I<MSEC> milliseconds to read event buffers

=item B<-I>, B<--perf-readout-interval> I<MSEC> (default: C<0>)

Wake up perf based monitors (e.g. sampling, metrics) at least every I<MSEC> milliseconds to read event buffers.
