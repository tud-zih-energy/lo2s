...
enum class LOCATION_TYPE : uint64_t
{
    CPU = 0,
    PROCESS = 1
};
...
{
public:
    RingbufWriter(ExecutionScope scope, std::string measurement_name)
    {
        ...
        memcpy(first_header->measurement_name, measurement_name.c_str(), measurement_name.size());

        if (scope.is_process())
        {
            first_header->loc_type = LOCATION_TYPE::PROCESS;
            first_header->pid = scope.as_process().as_pid_t();
        }
        else if (scope.is_cpu())
        {
            first_header->loc_type = LOCATION_TYPE::CPU;
            first_header->cpuid = scope.as_cpu().as_int();
        }
        else
        {
            throw std::runtime_error("Unallowed ExecutionScope type: " + scope.name());
        }
    ...
    uint64_t cctx_def(std::string func)
    {
        ...
        auto cctx = cctxs_.emplace(func, last_cctx_);
...
private:
    void write_fd()
    {
        ...
        *((int*)CMSG_DATA(cmptr)) = rb_->fd();
        ...
        // We need to include atleast one byte of sentinel data to be able
        // to discern between fd messages and EOF
        char foo = 42;
        iov[0].iov_base = &foo;
        iov[0].iov_len = 1;
        msg.msg_iov = iov;
...
public:
    ...
    ExecutionScope get_location()
    {
        switch (rb_->header()->loc_type)
        {
        case LOCATION_TYPE::CPU:
            return ExecutionScope(Cpu(rb_->header()->cpuid));
        case LOCATION_TYPE::PROCESS:
            return ExecutionScope(Process(rb_->header()->pid));
        }
