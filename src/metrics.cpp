...
metrics::metrics(otf2_trace& trace) : trace_(trace)
{
    // initialize each plugin, which is set using the SCOREP_METRIC_PLUGINS variables
    for (const auto& plugin : read_env_variables())
    {
        metric_plugins_.emplace_back(
            std::make_unique<metric_plugin>(plugin.first, plugin.second, trace_));
    }
