...
Record activity events (instruction samples or kernel execution information) for the given accelerator. Usable accelerators are "nvidia" for NVidia CUDA accelerators.
...
F</sys/kernel/tracing/events/I<E<lt>groupE<gt>>/I<E<lt>nameE<gt>>>.
...
on F</sys/kernel/tracing>.

To mount the tracefs execute:

    # mount -t tracefs none /sys/kernel/tracing

Non-root access requires you to change the ownership of the tracefs and execute with kernel.perf_event_paranoid set to -1:

    # chown -R myusername /sys/kernel/tracing
